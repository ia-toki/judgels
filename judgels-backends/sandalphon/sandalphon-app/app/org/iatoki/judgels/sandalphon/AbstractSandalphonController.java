...
public abstract class AbstractSandalphonController extends AbstractJophielClientController {
    ...
    protected Result renderTemplate(HtmlTemplate template) {
        ...
        if (IdentityUtils.getUserJid() != null) {
            template.addAdditionalScript(isLoggedOut.render(getUserIsLoggedInAPIEndpoint(), org.iatoki.judgels.jophiel.routes.JophielClientController.logout(getCurrentUrl(Http.Context.current().request())).absoluteURL(Http.Context.current().request(), Http.Context.current().request().secure()), "javascripts/isLoggedOut.js", SandalphonUtils.getRealUserJid()));
        }
